...
A secure shared wallet protocol that enables multiple users to contribute ETH to a common treasury, empowers the owner to grant withdrawal permissions and configurable spending allowances to trusted accounts, and securely manages fund distribution while protecting against unauthorized access, reentrancy attacks, and invalid state transitions.
 ...
 ## Project Overview
The `Shared Wallet Smart Contract` is a decentralized Ethereum-based treasury that enables multiple users to securely store and manage a common pool of Ether. While anyone can contribute funds to the wallet, only authorized accounts are permitted to withdraw, with each withdrawal constrained by an owner-defined spending allowance. </br>
...
* ****Event Logging:**** All critical operations are recorded through emitted events for transparency and off-chain monitoring.
* ****Gas-Efficient Error Handling:****  Utilizes custom errors to reduce deployment and transaction costs.
* ****Balance Query:**** Provides a view function for retrieving the wallet's current Ether balance.
...
* ****`SharedWallet.sol`****: The primary smart contract implementing a secure shared wallet that allows multiple users to deposit Ether while restricting withdrawals to authorized addresses
...
* ****`constructor()`:**** Initializes the Shared Wallet contract during deployment. This function is automatically executed once when the contract is deployed. It establishes the contract owner and grants the deployer permission to perform restricted operations. The function:
    ...
    * Can only be executed once during deployment.
...
* ****`allowance`:**** Records the maximum amount of Ether that each permitted address is authorized to withdraw from the shared wallet. The mapping associates a user's  address with a withdrawal allowance expressed in wei. Before processing any withdrawal,  the contract verifies that the caller's remaining allowance is at least equal to the requested amount. Upon a successful withdrawal, the corresponding allowance is automatically reduced by the amount withdrawn, preventing users from exceeding their assigned spending limit. This variable enables controlled  fund distribution by allowing administrators  to define individual withdrawal quotas without transferring ownership or full control of the wallet.
