...
* ****`allowance`:**** Records the maximum amount of Ether that each permitted address is authorized to withdraw from the shared wallet. The mapping associates a user's  address with a withdrawal allowance expressed in wei. Before processing any withdrawal,  the contract verifies that
